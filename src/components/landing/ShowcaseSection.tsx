  ...
  return (
    <section className="py-32 px-6 relative">
      <div className="max-w-7xl mx-auto">
        <ScrollReveal from={{ opacity: 0, y: 80, scale: 0.95 }} duration={1.2}>
                ...
                {[
                  ...
                  <div key={col.name} className="space-y-2.5">
                    ...
                    {Array.from({ length: col.count }).map((_, ti) => (
                      <motion.div
                        key={ti}
                        className="rounded-lg bg-background border border-border p-3 hover:border-primary/20 transition-colors"
                        initial={{ opacity: 0, y: 10 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
                        transition={{ delay: 0.5 + ti * 0.05, ease }}
                      >
                        <div
